  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">정산 완료 현장의 마감재를 조회하고 브랜드·발주처·제품으로 교차 검색합니다.</p>
      </div>
      ...
      <div className="relative mb-2">
        ...
        <input
          ...
          placeholder="브랜드·발주처·품번·제품명 검색 (예: 마페이, 윤현, LC산타베이지)"
      ...
      <p className="text-xs text-gray-400 mb-6">검색하면 여러 현장에 걸쳐 결과가 나옵니다 — AS 재주문 시 활용</p>
                ...
                return (
                  <tr key={f.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2.5">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${WORK_TYPE_COLOR[f.work_type] ?? 'bg-gray-100 text-gray-600'}`}>{f.work_type}</span>
                    </td>
                    <td className="px-4 py-2.5 text-gray-600 text-xs">{s?.project_name ?? '—'}</td>
                    <td className="px-4 py-2.5">
                      <Link href={`/finishes/${f.quote_id}`} className="text-blue-600 hover:text-blue-700 text-xs flex items-center gap-1">
          ...
          {sites.map(s => {
            ...
            return (
              <Link key={s.id} href={`/finishes/${s.id}`}
                className="flex items-center gap-4 bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-4 hover:border-blue-200 transition-colors">
                ...
                <div className="text-right">
                  <p className="text-xs text-gray-500">{c.total === 0 ? '미입력' : `${c.filled} / ${c.total} 항목`}</p>
                </div>
                <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${c.total === 0 ? 'bg-gray-100 text-gray-500' : done ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                  {c.total === 0 ? '시작 전' : done ? '입력완료' : '입력중'}
                </span>
